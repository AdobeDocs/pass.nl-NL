...
* **Gebeurtenissen die in de Stroom van de Authentificatie** worden geproduceerd (daadwerkelijke login met MVPD)
   ...
   * Melding van AuthN-poging - Deze wordt gegenereerd wanneer de gebruiker naar de MVPD-aanmeldsite wordt verzonden.
   * Melding van AuthN in behandeling - Als de gebruiker erin slaagt zich aan te melden met zijn MVPD, wordt dit gegenereerd wanneer de gebruiker wordt        omgeleid naar Adobe Pass Authentication.
...
In dit geval mag deduplicatie geen effect hebben omdat verschillende MVPD-gebruikers niet dezelfde gebruikersnaam mogen ontvangen. Wanneer u een bedrag voor twee verschillende merken maar dezelfde MVPD doet, zou het deduplicatie-effect veel groter moeten zijn.
...
Dit wordt automatisch geactiveerd wanneer `getAuthorization()` wordt aangeroepen en omvat alleen controles met Adobe Pass-verificatie. De MVPD is niet betrokken bij deze stroom.
...
Deze stroom omvat een retourtocht naar de MVPD.
...
De verificatiepooggebeurtenis wordt geactiveerd nadat de gebruiker op een bepaalde MVPD van de kiezer heeft geklikt.

* De eerste gebeurtenis aan MVPD-zijde die hier dichtbij ligt, is het laden van de pagina
...
De gebruiker is een bekende abonnee van de MVPD, meestal met een abonnement op Pay TV, maar soms met alleen internettoegang. Een geslaagde verificatie kan optreden omdat de gebruiker expliciet geldige gegevens heeft ingevoerd bij zijn MVPD of omdat de gebruiker eerder geldige gegevens had ingevoerd en &quot;onthoud mij&quot; was ingeschakeld (en de vorige sessie was niet verlopen).
...
Naast het krijgen van een authentificatietoken, moeten de gebruikers ook een toestemmingstoken krijgen alvorens inhoud te spelen. Dit gebeurt gewoonlijk na authentificatie, of als het toestemmingstoken verloopt. Aangezien deze controle op de server wordt uitgevoerd (van de Adobe Pass-verificatieservers tot de MVPD-servers), hoeft de gebruiker niets te doen.
...
Merk op dat niet alle MVPDs een afzonderlijke vergunningsstap steunt; voor sommige authentificatie wordt gelijkgesteld met vergunning. De MVPD verstuurt Adobe Pass Authentication als een succesvol antwoord op de backchannel AuthZ request en Adobe Pass Authentication maakt een AuthZ token.
...
* Aangezien AuthN en AuthZ tokens de reactie van MVPD voor een periode in het voorgeheugen onderbrengen, is de succesvolle gebeurtenis van het spelverzoek de meest frequente gebeurtenis in de metriek.
...
Deze waarde kan blijvend zijn op verschillende apparaten (de MVPD produceert dezelfde waarde voor een bepaalde gebruiker, ongeacht waar de aanmelding plaatsvindt) of tijdelijk (voor elke aanmelding wordt een nieuwe waarde gegenereerd, die de MVPD in de back-end toewijst. Doorgaans zijn de waarden die door MVPD&#39;s aan Adobe Pass-verificatie worden geleverd, blijvend in sessies en apparaten, maar zoals opgemerkt is de persistentie niet gegarandeerd en niet gevalideerd.
...
* AuthN toegekend
   * op dit moment identificeren we de gebruiker op basis van wat de MVPD retourneert - zodat het dagelijkse aantal unieke gebruikers met 1 wordt verhoogd
