...
In de onderstaande stappen wordt de volgorde van gebeurtenissen weergegeven met Adobe Pass-verificatie, wanneer een programmeur verificatie aanvraagt van een MVPD die SAML ondersteunt. De component Adobe Pass Authentication Access Enabler is actief op de client van de gebruiker/abonnee. Van daar, vergemakkelijkt de Toegankelijkheid alle stappen van de authentificatiestroom.
...
1. In de app van de SP wordt een &quot;MVPD Picker&quot; aan de gebruiker getoond om hun Pay TV provider (MVPD) te verkrijgen. SP richt dan browser van de gebruiker aan de geselecteerde dienst van de identiteitskaart van MVPD van de Identiteitsleverancier (IdP) opnieuw.  Dit is &quot;**programmeur-Toegelaten Login**&quot;.  MVPD verzendt het antwoord van IdP naar de Adobe die de dienst van de de assertieconsument van SAML, waar het wordt verwerkt.
...
Zoals die in de stappen hierboven wordt voorgesteld, tijdens de stroom AuthN moet een MVPD zowel een op SAML-Gebaseerd verzoek van AuthN goedkeuren als een reactie van SAML verzenden AuthN.

De [ Online van de Toegang van de Inhoud (OLCA) Authentificatie en de Specificatie van de Interface van de Authorization ](https://www.cablelabs.com/specifications/search?query=&category=&subcat=&doctype=&content=false&archives=false){target=_blanck} stelt een standaardverzoek en een reactie van AuthN voor. Hoewel de Authentificatie van Adobe Pass MVPDs niet vereist om hun machtigingsoverseinen op deze norm te baseren, kan het bekijken van de specificatie insight in de belangrijkste attributen verstrekken die voor een transactie AuthN worden vereist.

>[!NOTE]
>
>De AuthN-aanvraag die een MVPD ontvangt met Adobe Pass-verificatie, bevat wel een digitale handtekening. In het onderstaande voorbeeld ziet u echter geen handtekening, vanwege de beknoptheid. Voor een voorbeeld dat een digitale handtekening toont, zie het voorbeeld in [ de authentificatiereactie ](#authn-response) in de volgende secties.
...
| samlp:AuthnRequest | &lt;AuthnRequest> uitgegeven door de Serviceverlener aan de Identiteitsprovider. |
|-----------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| AssertionConsumerServiceURL | Dit is het Adobe-eindpunt dat moet worden gebruikt in de volgende respons. Standaardwaarde: **http://sp.auth.adobe.com/sp/saml/SAMLAssertionConsumer** |
...
| IssueInstant | Het tijdstip waarop het antwoord wordt gegeven. De tijdwaarde wordt gecodeerd in UTC zoals die in [ kern 2.0-os van SAML ](http://docs.oasis-open.org/security/saml/v2.0/saml-core-2.0-os.pdf){target=_blank} Sectie 1.3.3 wordt beschreven. |
...
| saml:Issuer | Identificeert de entiteit die het antwoordbericht heeft gegenereerd. (Voor meer informatie over dit element, zie SAML kern 2.0-os Sectie 2.2.5) |
| ds:Signature | Een handtekening van XML die de integriteit van beschermt en uitgever van de bewering voor authentiek verklaart, zoals die Sectie 5 in SAML kern 2.0-os wordt beschreven |
| samlp:NameIDPolicy | Geeft beperkingen op aan de naam-id die moet worden gebruikt om het gevraagde onderwerp weer te geven. |
...
Nadat de MVPD het verificatieverzoek heeft ontvangen en afgehandeld, moet deze nu een verificatierapport verzenden.
...
In de steekproef hierboven, verwacht Adobe SP om gebruikersidentiteitskaart uit Onderwerp/NameId te halen. Adobe SP kan worden gevormd om gebruiker te krijgen - identiteitskaart van een douane bepaald attribuut; de reactie zou een element zoals het volgende moeten bevatten:
...
| samlp:Response | Reactie ontvangen door Adobe Pass-verificatie. |
|------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Doel | Een URI-verwijzing die het adres aangeeft waarnaar deze aanvraag is verzonden. Dit is nuttig om kwaadwillig door:sturen van verzoeken aan onbedoelde ontvangers, een bescherming te verhinderen die door sommige protocolbanden wordt vereist. Als het aanwezig is, MOET de daadwerkelijke ontvanger controleren dat de verwijzing van URI de plaats identificeert waarop het bericht werd ontvangen. Als dit niet het geval is, MOET het verzoek worden afgewezen. Sommige protocolbanden kunnen het gebruik van dit attribuut vereisen. |
| ID | Een id voor de aanvraag. Het is van type xs :ID en MOET de vereisten volgen die in Sectie 1.3.4 van [ kern 2.0-os van SAML ](http://docs.oasis-open.org/security/saml/v2.0/saml-core-2.0-os.pdf){target=_blank} voor herkenningstekenuniciteit worden gespecificeerd. De waarden van het attribuut ID in een verzoek en het attribuut InResponseTo in de overeenkomstige reactie MOETEN aanpassen. |
...
| saml:Issuer | Identificeert de entiteit die het aanvraagbericht heeft gegenereerd. (Zie punt 2.2.5 voor meer informatie over dit element. SAML core 2.0-os) |
| samlp:Status | Een code die de status van het overeenkomstige verzoek vertegenwoordigt. |
| samlp:StatusCode | Een code die de status van de activiteit weergeeft die naar aanleiding van het overeenkomstige verzoek is uitgevoerd. |
| saml:Assertion | Dit type specificeert de basisinformatie gemeenschappelijk voor alle beweringen. |
...
| saml:Subject | Het onderwerp van de verklaring(en) in de bewering. |
| saml:NameID | Het &lt;NameID>-element is van het type NameIDType (zie Sectie 2.2.2 in SAML core 2.0-os) en wordt gebruikt in diverse SAML-assertieconstructies zoals de elementen &lt;Subject> en &lt;SubjectConfirmation> en in verschillende protocolberichten. |
...
| saml:SubjectConfirmation | Informatie aan de hand waarvan het onderwerp kan worden bevestigd. Indien meer dan één bevestiging van het onderwerp wordt verstrekt, volstaat het aan een van de onderwerpen te voldoen om het onderwerp voor de toepassing van de bewering te bevestigen. |
| saml:SubjectConfirmationData | Aanvullende bevestigingsinformatie die door een specifieke bevestigingsmethode moet worden gebruikt. De typische inhoud van dit element kan bijvoorbeeld een <!--<ds:KeyInfo>--> -element zijn, zoals gedefinieerd in de syntaxis en verwerkingsspecificatie voor XML-handtekeningen |
...
| saml:Conditions | Het element &lt;Condition> fungeert als een uitbreidingspunt voor nieuwe voorwaarden. |
| NotBefore | Het kenmerk NotBefore geeft het tijdmoment op waarop het geldigheidinterval begint. |
| saml:AudienceRestriction | Met het element &lt;AudienceRestriction> wordt opgegeven dat de bewering is gericht tot een of meer specifieke doelgroepen die worden aangeduid door elementen van &lt;Audience>. |
| saml:Audience | Een URI-verwijzing die een doelgroep identificeert. |
...
| saml:AuthnContext | De context die door de authenticerende autoriteit tot en met de authentificatiegebeurtenis wordt gebruikt die deze verklaring voortbracht. |
| saml:AuthnContextClassRef | Een verwijzing van URI die een klasse van de authentificatiecontext identificeert die de verklaring van de authentificatiecontext beschrijft die volgt. |
